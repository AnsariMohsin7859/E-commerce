const SubCatModel = require("../model/subcategory.model")
...
const updateSubCatName = (req, res, next) => {
    ...
    SubCatModel.findOneAndUpdate({
        _id: catId
    }, dataToUpdate, (err, data) => {
        if (err) {
            return res.status(400).json({
                error: err,
...
const updateSubCatStatus = (req, res, next) => {
    ...
    SubCatModel.findOneAndUpdate({
        ...
        if (err) {
            ...
            return res.status(200).json({
                msg: "Category status been updated successfully! as " + status
            })
            // next()
