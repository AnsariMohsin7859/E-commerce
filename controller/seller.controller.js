...
const sendSellerData = (req , res)=>{
    ...
    let doc = new sellerModel({ name : sellerName , email :  sellerEmail  ,
        password : dateCreated ,  number: sellerNumber ,shopeName: sellerShopeName ,address: sellerAddress ,status: sellerStatus 
    });//insert , update
    // doc.save((err , data)=>{
    //     if(err)
    //     {
    ...
    resp.then((data)=>{
        ...
        return res.json({result : data , msg : "seller added succesfully"})
    }).catch((err)=>{
        console.log("catch inside");
        if(err.code == 11000)
        {
            return res.send({ msg : "already exist seller" , error : err})
...
const getAllSeller = async (req , res )=>{
    ...
    let skip= page * limit;
    sellerModel.find().limit(limit).skip(skip)
    .then((data)=>{
        return res.status(200).json({total : data.length  , msg : "successfully got seller list" , result : data })
    })
    .catch(  (err)=>{          return res.status(400).json({error : err , msg : "failed to get seller list"})}
    )
}
...
console.log(data);
        ...
        return res.status(200).json({ msg : "successfully got seller" , result : data })
    })
    .catch(  (err)=>{     
        console.log(err);
        return res.status(400).json({error : err , msg : "failed to get seller"})}
    )
}
...
const deleteSeller =  (req , res )=>{
    ...
    sellerModel.findByIdAndDelete({_id: sellerid}, (err, result)=>{
        if (err) {
            return res.status(400).json({reslt : result, msg : "unable to find and delete seller"})
        } else {
            if (result == null) {
            
                return res.status(201).json({msg : "seller does not exist"})
            } else {
            return res.status(200).json({result : result , msg : "successfully delete seller"})
...
const updateSellerDetails = (req , res , next )=>{
    ...
    //1 where , 2 set : what to update
        sellerModel.findOneAndUpdate(  { _id : Id } , dataToUpdate , (err , data)=>{
            if(err)
        {
            return res.status(400).json({error : err , msg : "Your request could not be processed. Please try again."})
        }else{
                return res.status(200).json({ msg : "seller details has been updated successfully!"})
    ...
    //1 where , 2 set : what to update
            ...
            if(err)
        {
            return res.status(400).json({error : err , msg : "Your request could not be processed. Please try again."})
                ...
                return res.status(200).json({ msg : "Seller status been updated successfully! as "+status})
